...
use axum::{Json, Router, http::{HeaderValue, Method, header}, routing::get};
use serde::Serialize;
...
async fn main() {
    ...
    let db=PgPoolOptions::new().max_connections(10).connect(&database_url).await.expect("failed to connect to PostgreSQL");
    sqlx::migrate!().run(&db).await.expect("failed to run database migrations");
    let state=AppState{db,jwt_secret:jwt_secret.into_bytes()};
    ...
    axum::serve(listener,app(state)).with_graceful_shutdown(shutdown_signal()).await.expect("API server failed");
}
