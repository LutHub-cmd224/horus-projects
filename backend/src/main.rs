...
async fn health() -> Json<HealthResponse> {
    Json(HealthResponse { status: "ok", service: "horus-api" })
}
...
async fn main() {
    tracing_subscriber::fmt()
        .with_env_filter(tracing_subscriber::EnvFilter::try_from_default_env().unwrap_or_else(|_| "horus_api=debug,tower_http=debug".into()))
        .init();
