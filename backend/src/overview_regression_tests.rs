...
async fn overview_is_200_when_model_is_current_phase() {
    ...
    let app = app(AppState {
        db,
        jwt_secret: b"ci-overview-regression-secret-32-bytes".to_vec(),
    });
    ...
    let (status, _) = request(
        &app,
        "PUT",
        &format!("/api/v1/phases/{analyze_id}/analyze"),
        Some(token),
        Some(json!({
            "problem":"Trouver le bon professionnel",
            "target_audiences":["Particulier"],
            "target_details":"Besoin précis",
            "value_proposition":"Mise en relation pertinente",
            "success_objectives":["Obtenir une proposition"],
            "budget":"Limité",
            "deadline":"3 mois",
            "platform":"Web",
            "special_constraints":"Confiance et vie privée",
            "constraints_unknown":false,
            "mvp_features":["Publier un besoin","Recevoir une proposition"]
        })),
    )
    .await;
    ...
    let (status, _) = request(
        &app,
        "POST",
        &format!("/api/v1/phases/{analyze_id}/validate"),
        Some(token),
        Some(json!({"comment":"Analyse terminée"})),
    )
    .await;
