...
describe("ModelWorkspace", () => {
  ...
  it("generates model artifacts", async () => {
    vi.spyOn(api, "model").mockResolvedValue(data);
    const generate = vi
      .spyOn(api, "generateModelArtifact")
      .mockResolvedValue({});
    render(<ModelWorkspace onChanged={vi.fn()} phase={phase} token="token" />);
    fireEvent.click(
      await screen.findByRole("button", { name: /générer mcd/i }),
    );
    await waitFor(() =>
      expect(generate).toHaveBeenCalledWith("model", "MCD", "token"),
    );
  });

  it("validates the phase after MCD, MLD and MPD are ready", async () => {
    vi.spyOn(api, "model").mockResolvedValue({
      ...data,
      artifacts: ["MCD", "MLD", "MPD"],
    });
    const validate = vi
    ...
    fireEvent.click(
      await screen.findByRole("button", { name: /valider modéliser/i }),
    );
    await waitFor(() =>
      expect(validate).toHaveBeenCalledWith(
        "model",
        "token",
        "Validation MCD → MLD → MPD",
      ),
    );
  });
});
