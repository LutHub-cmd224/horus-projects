...
export function AnalyzeWorkspace({ phase, token, onChanged }: AnalyzeWorkspaceProps) {
  ...
  async function loadCriteria() {
    ...
    setError('');
    try {
      setCriteria(await api.criteria(phase.id, token));
    } catch {
        ...
        <div>
          ...
          <div className="mt-8 rounded-2xl bg-[#1b1b18] p-5 text-white">
            <div className="flex items-end justify-between gap-4">
              <div><p className="text-xs uppercase tracking-[0.16em] text-white/45">Progression requise</p><p className="mt-2 text-sm text-white/60">{completed} / {required.length} critères</p></div>
              <p className="text-4xl font-semibold">{required.length ? Math.round((completed / required.length) * 100) : 0}%</p>
            </div>
            <div className="mt-6 h-1 overflow-hidden bg-white/15"><div className="h-full bg-white transition-all" style={{ width: `${required.length ? (completed / required.length) * 100 : 0}%` }} /></div>
          </div>
        </div>

        <div>
          {loading ? <div className="grid min-h-64 place-items-center text-sm text-black/40">Chargement des critères…</div> : (
            <div className="space-y-3">
              {criteria.map((criterion, index) => (
                <button
                  ...
                  disabled={phase.status === 'VALIDATED' || busyId === criterion.id}
                  key={criterion.id}
                  onClick={() => void toggleCriterion(criterion)}
                  type="button"
                >
                  <span className={`grid size-8 shrink-0 place-items-center rounded-full border text-xs font-semibold ${criterion.completed ? 'border-white/20 bg-white text-black' : 'border-black/15'}`}>{criterion.completed ? '✓' : String(index + 1).padStart(2, '0')}</span>
                  <span className="flex-1"><span className="block text-sm font-semibold">{criterion.label}</span><span className={`mt-1 block text-[11px] uppercase tracking-[0.14em] ${criterion.completed ? 'text-white/45' : 'text-black/35'}`}>{criterion.required ? 'Requis' : 'Optionnel'} · {criterion.code}</span></span>
                </button>
              ))}
            </div>
          )}
          ...
          <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <p className="text-xs text-black/40">{phase.status === 'VALIDATED' ? 'Analyse validée. Modéliser est maintenant disponible.' : canValidate ? 'Tous les critères requis sont terminés.' : 'Complétez les critères requis pour valider.'}</p>
            <button className="rounded-full bg-[#d9503f] px-6 py-3 text-sm font-bold text-white disabled:cursor-not-allowed disabled:opacity-35" disabled={!canValidate || validating} onClick={() => void validate()} type="button">{validating ? 'Validation…' : phase.status === 'VALIDATED' ? 'Phase validée' : 'Valider Analyser →'}</button>
          </div>
        </div>
      </div>
    </section>
